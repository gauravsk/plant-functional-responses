...
if(thinkpad){
    compdens <- read.csv("data/performance/calculated/NCompetitors_per_plot.csv")
  seedprod <- read.csv("data/performance/seed_production_processed.csv", stringsAsFactors = F)
...
for(sp in unique(seedprod$sp_code)) {
  ...
  # make a vector named `densvector` for use later
  densvector <- seedprod %>% filter(sp_code == sp) %>% dplyr::select(N_per_neighborhood) %>% unlist %>% as.numeric
  # make a which_plot vector for later
  which_plot <-   seedprod %>% filter(sp_code == sp) %>% dplyr::select(plot_num) %>% unlist %>% as.numeric
  ...
  # Predicted = ...
  pred <- mod2pars[sp,"lambda"]/(1+mod2pars[sp,"alpha"]*densvector)
  ...
  predicted_suppression <- data.frame(plot = rep(unique(which_plot), each = length(tmpx)),
                                      density = rep(tmpx, length(unique(which_plot))),
  ...
  # cowplot::plot_grid(gg_mod1_predVobs, gg_mod2_predVobs, gg_mod3_predVobs, gg_mod4_predVobs, gg_mod5_predVobs,gg_mod5_predVdens
  plot_to_save <- cowplot::plot_grid(gg_mod5_predVobs, gg_mod5_predVdens, rel_widths = c(2,3))
  ggsave(plot_to_save, filename = paste0("figs/scripted_figs/mod5_performance_",sp,".png"), width = 13, height = 7)
  
  # Compute some AICs
  AIC1<-2*(testcomp1$value+length(testcomp1$par)) #2*log likelihood + numpar
  AIC2<-2*(testcomp2$value+length(testcomp2$par))
  AIC3<-2*(testcomp3$value+length(testcomp3$par))
  AIC4<-2*(testcomp3$value+length(testcomp4$par))
  AIC5<-2*(testcomp3$value+length(testcomp4$par))
  
  aics_to_print <- cbind(species = rep(sp, 5), model = paste("Model",1:5), aic = c(AIC1, AIC2, AIC3, AIC4, AIC5))
  write.csv(aics_to_print, file = paste0("model_outputs/", sp,"_aics.csv"))
}
